  ...
  build_oplus_odm:
    ...
    steps:
      ...
      - name: Install Android ext4 tools
        run: |
          # Install make_ext4fs (handles SELinux contexts natively)
          sudo apt-get install -y android-sdk-libsparse-utils android-sdk-ext4-utils 2>/dev/null || true

          # Check if available from packages
          if command -v make_ext4fs &>/dev/null; then
            echo "✓ make_ext4fs available from packages"
          else
            echo "Building from CyanogenMod source..."
            sudo apt-get install -y build-essential libsparse-dev zlib1g-dev 2>/dev/null || true
            git clone --depth=1 https://github.com/nicholasgc/android_system_extras_ext4_utils -b main /tmp/ext4_utils 2>/dev/null || \
            git clone --depth=1 https://github.com/nicholasgc/ext4_utils -b master /tmp/ext4_utils 2>/dev/null || {
              echo "⚠️ ext4_utils clone failed — will fall back to mkfs.ext4"
              exit 0
            }
            cd /tmp/ext4_utils
            make -j$(nproc) 2>/dev/null || gcc -o make_ext4fs make_ext4fs.c *.c -lsparse -lz 2>/dev/null || {
              echo "⚠️ make_ext4fs build failed — will fall back to mkfs.ext4"
              exit 0
            }
            sudo cp make_ext4fs /usr/local/bin/ 2>/dev/null || true
            sudo cp e2fsdroid /usr/local/bin/ 2>/dev/null || true
            cd -
            rm -rf /tmp/ext4_utils
          fi
