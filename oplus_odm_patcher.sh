...
mount_odm() {
    ...
    if [ "$fs_type" = "erofs" ]; then
        ...
        if command -v extract.erofs &>/dev/null; then
            ...
            log_error "No EROFS extraction tool found (need extract.erofs or fsck.erofs)"
            exit 1
        fi
        log_success "$label ODM extracted (EROFS → directory)"
    else
        log_info "Mounting $label ODM (ext4)..."
        mount -o loop,rw "$odm_img" "$mount_point"
        if [ $? -ne 0 ]; then
            log_warning "RW mount failed, trying RO..."
            mount -o loop,ro "$odm_img" "$mount_point"
            if [ $? -ne 0 ]; then
                log_error "Failed to mount $label odm.img"
                exit 1
            fi
        fi
